...
namespace WoWAddonIDE.Services
{
    public static class GitHubAuthService
    {
            ...
            CancellationToken ct)
        {
            ...
            {
                try { ctx.Response.Close(); } catch (Exception ex) { LogService.Warn("Failed to close OAuth response", ex); }
            }
        }
            ...
            CancellationToken ct)
        {
            ...
            if (!resp.IsSuccessStatusCode)
            {
                ...
                catch (Exception ex) { LogService.Warn("Failed to parse GitHub token error response", ex); }
            ...
            {
                LogService.Warn("Failed to parse GitHub token response", ex);
            }
        ...
        private static async Task<HttpListenerContext?> WaitForContextAsync(HttpListener listener, CancellationToken ct)
        {
            ...
            using (ct.Register(() => { try { listener.Stop(); } catch (Exception ex) { LogService.Warn("Failed to stop OAuth listener on cancellation", ex); } }))
            {
                try { return await t.ConfigureAwait(false); }
                catch (Exception ex) { LogService.Warn("OAuth callback listener cancelled or failed", ex); return null; }
            }
        }
