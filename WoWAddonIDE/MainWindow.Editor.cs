...
namespace WoWAddonIDE
{
    public partial class MainWindow : Window
    {
        ...
        // Guard so we only wire global keys once
        private bool _inputKeysWired = false;
        ...
        private void OpenFileInTab(string path)
        {
            ...
            // Global shortcuts (wire once)
            if (!_inputKeysWired)
            {
                // Ctrl+F — Find
                this.InputBindings.Add(new KeyBinding(
                    new RelayCommand(_ => ShowFindWindow(false)),
                    new KeyGesture(Key.F, ModifierKeys.Control)));

                // Ctrl+H — Replace
                this.InputBindings.Add(new KeyBinding(
                    new RelayCommand(_ => ShowFindWindow(true)),
                    new KeyGesture(Key.H, ModifierKeys.Control)));

                // Ctrl+G — Go To Line
                this.InputBindings.Add(new KeyBinding(
                    new RelayCommand(_ => GoToLine_Prompt()),
                    new KeyGesture(Key.G, ModifierKeys.Control)));

                // Ctrl+Shift+I — Format Document
                this.InputBindings.Add(new KeyBinding(
                    new RelayCommand(_ => FormatActiveDocument()),
                    new KeyGesture(Key.I, ModifierKeys.Control | ModifierKeys.Shift)));

                _inputKeysWired = true;
            }
        ...
        {
            try { /* no-op here; settings dialog persists to disk */ } catch { }
        }
    }
}