    ...
    public partial class MainWindow : Window
    {
        ...
        private void CloseTab(TabItem tab)
        {
            ...
            if (isDirty)
            {
                ...
                {
                    try { SaveTab(tab); } catch (Exception ex) { LogService.Warn("CloseTab: failed to save tab before closing", ex); }
                }
            }

            // Remember closed tab path for Ctrl+Shift+T
            if (tab.Tag is string path && !string.IsNullOrWhiteSpace(path))
                _closedTabPaths.Push(path);

            // Clean up folding manager
            var editor = FindEditorIn(tab.Content);
            if (editor != null) Folding_RemoveForEditor(editor);
